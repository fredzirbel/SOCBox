# SOC Box — Project Context & Status

**SOC Box (The SOC Analyst's Toolbox)** — a containerized URL/phishing analysis
...
- **Repo:** https://github.com/fredzirbel/SOCBox (branch `main`, all work pushed)
- **Local:** `C:\Users\Freddy\Desktop\Projects\SOCBox`
...
analysts. SOC Box already beats SlashNext on its two worst pain points: it gets past
...
- **Dev server:** `SOCBOX_PORT=8015 .venv/Scripts/python.exe -m socbox.web.app --no-reload`
...
- **Auth to run the server:** OIDC env (or `SOCBOX_AUTH_DEV=1` for an insecure local spin-up) —
  the server fail-closes otherwise.
- **CLI:** `socbox <url>` · `socbox <url> -i` (interactive CAPTCHA) · `python -m socbox.feeds_import ...`
...
## Architecture
- `src/socbox/scanner.py` — orchestrates analyzers (thread pool + a persistent thread-local
  Playwright browser via `_get_browser`), scoring, classification, screenshots. Emits SSE
  events through an `on_event` callback.
- `src/socbox/analyzers/` — 8 analyzers (url_lexical, whois_dns, ssl_tls, http_response,
  page_content, link_discovery, download, threat_feeds). `page_content` also stashes
  `page_text` + scripts for classification.
- `src/socbox/feeds/` — VirusTotal, AbuseIPDB, Google Safe Browsing.
- `src/socbox/browser.py` — Playwright launch, Cloudflare bypass, **CAPTCHA solving**, human-like
...
- `src/socbox/dns_util.py` — DoH resolution (`resolve_host`/`resolve_hostname`), Chromium
...
- `src/socbox/scoring.py` — `calculate_score` + `_composite_parts` (shared math) +
  `score_breakdown` (per-analyzer contributions; 45% analyzers / 55% feeds blend + VT floor).
- `src/socbox/classification.py` — rule-based ATT&CK technique tagging.
- `src/socbox/store.py` — **SQLite** persistence (scans + TP/BTP/FP dispositions; queryable
  verdict history). DB at `screenshots/socbox.db` (gitignored, on the mounted volume).
- `src/socbox/feeds_import.py` — `socbox-feeds` CLI; pulls live URLs from URLhaus (authed API)
  + OpenPhish.
- `src/socbox/web/app.py` — FastAPI: web routes, SSE `/stream/{id}`, REST + **v1 TAP API**.
- `src/socbox/web/templates/results.html` — **TWO render paths that must stay in sync**:
...
- **AuthN/AuthZ** ([web/auth.py](src/socbox/web/auth.py)): analysts log in via **OIDC SSO**
  ...
  (auto-login) requires `SOCBOX_AUTH_DEV=1`. Server **fail-closes** (main() exits) if oidc mode
  is missing config.
- **SSRF guard** ([netguard.py](src/socbox/netguard.py)): every scan-entry rejects (400) targets
...
- **Secure headers** (CSP w/ Google-Fonts + noVNC `frame-src`; X-Frame-Options, nosniff,
  ...
  gitleaks).
- ⚠️ **Secrets via env only** (never committed): `SOCBOX_SESSION_SECRET`,
  `SOCBOX_OIDC_CLIENT_SECRET`, `SOCBOX_API_TOKENS`. Compose passes them through.
...
9. **`socbox-feeds`** — import live malicious URLs (URLhaus authed API + OpenPhish): CLI,
...
1. **Cue-aware download poll** ([analyzers/download.py](src/socbox/analyzers/download.py)) —
...
2. **Dropped reverse-DNS PTR check** ([analyzers/whois_dns.py](src/socbox/analyzers/whois_dns.py))
...
- 🔴 **SSL/TLS analyzer was inert on HTTPS** ([ssl_tls.py](src/socbox/analyzers/ssl_tls.py)) —
...
- 🔴 **Stored XSS in the analyst report** ([web/app.py](src/socbox/web/app.py)) — `report_json`
...
- 🟠 SSRF hardening: DoH fallback refuses non-public results (`is_public_ip` in
  [dns_util.py](src/socbox/dns_util.py)); async webhook `callback_url` is validated through the
  ...
  now opt-in (`--reload`/`SOCBOX_RELOAD=1`), not the prod default.
...
- ~~**DNS robustness**~~ ✅ **DONE** (all three): `dns: [1.1.1.1, 8.8.8.8]` on the `socbox`
...
- Never commit `config/local.yaml` or `screenshots/socbox.db` (both gitignored).
