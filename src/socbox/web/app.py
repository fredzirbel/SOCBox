"""FastAPI web application for SOC Box."""
...
from socbox import store
from socbox.config import get_api_key, load_config
from socbox.enrich import enrich_ips
from socbox.feeds.virustotal import scanned_engine_total
from socbox.netguard import target_block_reason
from socbox.scanner import scan_url, shutdown_browser
from socbox.web.auth import (
    SecurityHeadersMiddleware,
    build_csp,
    init_auth,
    verify_auth_or_exit,
)
from socbox.web.defang import defang as defang_url
from socbox.web.osint import generate_osint_links
...
_scan_executor = ThreadPoolExecutor(
    max_workers=_MAX_CONCURRENT_SCANS, thread_name_prefix="socbox-scan",
)
...
app = FastAPI(title="SOC Box", version="0.1.0", lifespan=_lifespan)
...
_DB_PATH = _SCREENSHOT_DIR / "socbox.db"  # durable scans + dispositions
...
def _resolve_ip(url: str) -> str:
    ...
    """
    from socbox.dns_util import resolve_hostname
...
def scan(request: Request, url: str = Form(...)) -> RedirectResponse:
    """Accept a URL, run the SOC Box scan, and redirect to results.
...
# ---------------------------------------------------------------------------
# KQL hunting queries (verdict/artifact-aware, auto-generated)
# ---------------------------------------------------------------------------


@app.get("/api/kql/{scan_id}")
async def api_kql(scan_id: str) -> JSONResponse:
    """Return verdict- and artifact-aware KQL hunting queries for a scan.

    The query set is chosen from the scan's verdict + artifacts: phishing
    URLs get URL-click / email / sign-in queries; file downloads get endpoint
    (Device*) queries pivoting on the file hash/name + hosting domain/IP, plus
    the delivery queries. The escalation write-up is produced by the SOC's
    separate investigative agent, not here.

    Args:
        scan_id: The unique scan identifier.

    Returns:
        JSON with ``kql_queries`` (list of ready-to-paste KQL queries).
    """
    ...
    report = entry["report"]
    file_dl = report.file_download
    kql_queries = generate_kql_queries(
        domain=entry.get("domain", ""),
        url=report.url,
        category=report.risk_category.value,
        file_download=asdict(file_dl) if file_dl else None,
        resolved_ip=entry.get("ip", ""),
    )

    return JSONResponse({"kql_queries": kql_queries})
...
def api_feeds_import(source: str = "urlhaus", limit: int = 15, tag: str = "") -> JSONResponse:
    ...
    """
    from socbox.feeds_import import fetch_openphish, fetch_urlhaus, urlhaus_auth_key
...
def main() -> None:
    """Start the SOC Box web server."""
    import sys

    # Fail-closed: refuse to start a real server with misconfigured OIDC auth.
    verify_auth_or_exit(_config)

    # Auto-reload is OPT-IN (--reload or SOCBOX_RELOAD=1). It spawns a reloader
    # subprocess, which is wrong for production: SOC Box holds scan/job/stream state
    ...
    reload_requested = "--reload" in sys.argv or os.getenv(
        "SOCBOX_RELOAD", "").strip().lower() in ("1", "true", "yes")
    is_dev = reload_requested and "--no-reload" not in sys.argv

    host = os.getenv("SOCBOX_HOST", "0.0.0.0")  # nosec B104 - containerized server, bind intended
    port = int(os.getenv("SOCBOX_PORT", "8000"))
    uvicorn.run(
        "socbox.web.app:app",
        host=host,
        port=port,
