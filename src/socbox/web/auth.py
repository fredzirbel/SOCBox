...
def oidc_missing(cfg: dict[str, Any]) -> list[str]:
    ...
    if not oidc.get("client_secret"):
        missing.append("oidc.client_secret (SOCBOX_OIDC_CLIENT_SECRET)")
    if not _auth(cfg).get("session_secret"):
        missing.append("session_secret (SOCBOX_SESSION_SECRET)")
    return missing
