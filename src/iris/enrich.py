"""Multi-source IP enrichment for IRIS.
...
These are *reputation lookups about* the IP (to trusted OSINT APIs); IRIS never
...
def _maxmind_geo(ip: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    ``enrich.maxmind_db`` points at a GeoLite2-City.mmdb, so IRIS never requires
...
def enrich_ips(ips: list[str], config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    Args:
        ...
        config: The loaded IRIS configuration.
