"""Threat-intel feed importer for IRIS.
...
Typical use::
    ...
    python -m iris.feeds_import --source urlhaus --limit 20 --tag ClearFake
    python -m iris.feeds_import --source openphish --limit 30 --output urls.txt
...
def urlhaus_auth_key(config: dict | None = None) -> str:
    ...
    Args:
        config: Optional loaded IRIS config dict.
